...
#[must_use]
pub fn map_chunk_coords(half_extent: f32) -> Vec<IVec2> {
    let span = ELEV_CHUNK_CELLS as f32 * ELEVATION_CELL;
    let min = (-half_extent / span).floor() as i32;
    let max = (half_extent / span).floor() as i32;
    (min..=max)
...
mod tests {
    ...
    fn map_chunk_coords_covers_full_map_row_major() {
        ...
        let coords = map_chunk_coords(2000.0);
