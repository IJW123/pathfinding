...
/// World-space `(min, max)` bounds for the broad phase. Circle = ±radius; Obb uses the closed-form
/// rotated-box extent; Convex takes min/max of its rotated, translated vertices (off-center safe).
#[must_use]
pub fn world_aabb(shape: &ColliderShape, transform: &Transform) -> (Vec2, Vec2) {
    ...
    match shape {
        ...
        ColliderShape::Convex { hull } => world_poly(hull.points(), transform).iter().fold(
...
mod tests {
    ...
    fn aabb_axis_aligned_obb() {
        ...
        assert!(close_vec(min, Vec2::new(-1.0, -2.0)));
        assert!(close_vec(max, Vec2::new(1.0, 2.0)));
    }
    ...
    fn aabb_45deg_square_grows_by_sqrt2() {
        ...
        let s = 2.0_f32.sqrt();
        assert!(close_vec(min, Vec2::splat(-s)));
        assert!(close_vec(max, Vec2::splat(s)));
    }

    #[test]
    fn aabb_circle_rotation_invariant() {
        let tx = Transform::from_xyz(3.0, 0.0, 0.0).with_rotation(Quat::from_rotation_z(FRAC_PI_4));
        let (min, max) = world_aabb(&ColliderShape::Circle { radius: 1.0 }, &tx);
        assert!(close_vec(min, Vec2::new(2.0, -1.0)));
        assert!(close_vec(max, Vec2::new(4.0, 1.0)));
    }
    ...
    fn aabb_off_center_convex() {
        ...
        let (min, max) = world_aabb(&ColliderShape::Convex { hull }, &tx);
        assert!(close_vec(min, Vec2::new(10.0, 10.0)));
        assert!(close_vec(max, Vec2::new(12.0, 11.0)));
    }
}
